	...
	public abstract class AIBase : VersionedMonoBehaviour {
		...
		public Vector3 target {
			get {
				...
				return setter != null ? setter.target : Vector3.negativeInfinity;
			}
			set {
				targetCompatibility = Vector3.negativeInfinity;
				var setter = GetComponent<AIDestinationSetter>();
		...
		protected virtual void FixedUpdate () {
			if (!(rigid == null && rigid2D == null) && canMove) {
				Vector3 nextPosition;
				// Quaternion nextRotation;
				MovementUpdate(Time.fixedDeltaTime, out nextPosition);
				FinalizeMovement(nextPosition);
			}
		}
		...
		public void MovementUpdate (float deltaTime, out Vector3 nextPosition) {
			...
			MovementUpdateInternal(deltaTime, out nextPosition);
		}
		...
		public virtual void FinalizeMovement (Vector3 nextPosition) {
		...
		protected override int OnUpgradeSerializedData (int version, bool unityThread) {
			...
			if (unityThread && targetCompatibility != Vector3.negativeInfinity) target = targetCompatibility;
