	...
	public class AILerp : VersionedMonoBehaviour, IAstarAI {
		...
		public Vector3 target {
			get {
				...
				return setter != null ? setter.target : new Vector3(float.NegativeInfinity, float.NegativeInfinity, float.NegativeInfinity);
			}
			set {
				targetCompatibility = new Vector3(float.NegativeInfinity, float.NegativeInfinity, float.NegativeInfinity);
				var setter = GetComponent<AIDestinationSetter>();
				if (setter == null) setter = gameObject.AddComponent<AIDestinationSetter>();
				setter.target = value;
				destination = value != new Vector3(float.NegativeInfinity, float.NegativeInfinity, float.NegativeInfinity) ? value : new Vector3(float.PositiveInfinity, float.PositiveInfinity, float.PositiveInfinity);
			}
		}
		...
		protected virtual void Update () {
			...
			if (canMove) {
				...
				MovementUpdate(Time.deltaTime, out nextPosition);
				FinalizeMovement(nextPosition);
			}
		}

		/// <summary>\copydoc Pathfinding::IAstarAI::MovementUpdate</summary>
		public void MovementUpdate (float deltaTime, out Vector3 nextPosition) {
		...
		public void FinalizeMovement (Vector3 nextPosition) {
		...
		protected override int OnUpgradeSerializedData (int version, bool unityThread) {
			...
			if (unityThread && targetCompatibility != new Vector3(float.NegativeInfinity, float.NegativeInfinity, float.NegativeInfinity)) target = targetCompatibility;
