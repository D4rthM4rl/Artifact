...
namespace Pathfinding {
	...
	public partial class AIPath : AIBase, IAstarAI {
		...
		/// <summary>
		/// Can only start if enabled, so that it doesn't try to look for pathfinding graphs before there are any
		/// </summary>
		...
		/// <summary>
    /// Returns the next point on the path that the AI should move to.
    /// </summary>
    public Vector3 GetNextPoint() {
        // if (!interpolator.valid) {
        //     // If there's no valid path, return the current position
        //     return position;
        // }

        // // Calculate the next waypoint distance from the current position
        // float distanceToNextWaypoint = pickNextWaypointDist;
        
        // // Advance the interpolator until it's at the distance to the next waypoint
        // interpolator.distance = (interpolator.distance + distanceToNextWaypoint);
        
        // // Return the next position the AI should move to
		...
		protected override void MovementUpdateInternal (float deltaTime, out Vector3 nextPosition) {
			...
			// velocity2D = MovementUtilities.ClampVelocity(velocity2D, maxSpeed, slowdown, slowWhenNotFacingTarget && enableRotation, forwards);
				...
				#region Actually moving
			// GetComponent<Rigidbody2D>().AddForce(velocity2D);
			...
			// CalculateNextRotation(slowdown);
		}
