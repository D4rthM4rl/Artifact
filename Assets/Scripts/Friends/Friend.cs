...
public class Friend : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Vector2 aim = mousePos - new Vector2(transform.position.x, transform.position.y);
            Fire(aim);
        ...
        if (movement.x != 0 && movement.y != 0)
        {
            ...
            GetComponent<Rigidbody2D>().AddForce(direction * (friend.speed * 0.35f));
            lastOffsetX = offsetX;
            lastOffsetY = offsetY;
        }
        else {
            ...
            {
                Vector2 direction = (new Vector3(player.transform.position.x - lastOffsetX, player.transform.position.y - lastOffsetY) - transform.position).normalized;
